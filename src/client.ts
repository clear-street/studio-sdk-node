...
export interface ClientOptions {
   ...
   * Set the log level.
   *
   * Defaults to process.env['STUDIO_SDK_LOG'] or 'warn' if it isn't set.
   */
...
export class StudioSDK {
   ...
   */
  constructor({
    baseURL = readEnv('STUDIO_SDK_BASE_URL'),
    bearerToken = readEnv('STUDIO_SDK_BEARER_TOKEN'),
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? StudioSDK.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('STUDIO_SDK_LOG'), "process.env['STUDIO_SDK_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('STUDIO_SDK_CUSTOM_HEADERS');
    if (customHeadersEnv) {
