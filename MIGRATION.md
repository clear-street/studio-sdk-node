...
This guide outlines the changes and steps needed to migrate your codebase to the latest version of the Studio SDK TypeScript SDK.
...
To use it, upgrade the `@clear-street/studio-sdk` package, then run `./node_modules/.bin/clear-street-studio-sdk migrate ./your/src/folders` to update your code.
...
- `client.entities.regtMarginSimulations.retrieve()`
- `client.accounts.orders.retrieve()`
- `client.accounts.orders.cancel()`
- `client.accounts.orders.patch()`
- `client.accounts.trades.retrieve()`
- `client.accounts.positions.retrieve()`
- `client.accounts.locateOrders.retrieve()`
- `client.accounts.locateOrders.update()`
- `client.accounts.inventories.retrieve()`
...
#### Refactor of `@clear-street/studio-sdk/core`, `error`, `pagination`, `resource` and `uploads`

Much of the `@clear-street/studio-sdk/core` file was intended to be internal-only but it was publicly accessible, as such it has been refactored and split up into internal and public files, with public-facing code moved to a new `core` folder and internal code moving to the private `internal` folder.
...
```typescript
// Before
import '@clear-street/studio-sdk/error';
import '@clear-street/studio-sdk/pagination';
import '@clear-street/studio-sdk/resource';
import '@clear-street/studio-sdk/uploads';

// After
import '@clear-street/studio-sdk/core/error';
import '@clear-street/studio-sdk/core/pagination';
import '@clear-street/studio-sdk/core/resource';
import '@clear-street/studio-sdk/core/uploads';
```

If you were relying on anything that was only exported from `@clear-street/studio-sdk/core` and is also not accessible anywhere else, please open an issue and we'll consider adding it to the public API.

#### Resource classes

Previously under certain circumstances it was possible to import resource classes like `Entities` directly from the root of the package. This was never valid at the type level and only worked in CommonJS files.
Now you must always either reference them as static class properties or import them directly from the files in which they are defined.

```typescript
// Before
const { Entities } = require('@clear-street/studio-sdk');

// After
const { StudioSDK } = require('@clear-street/studio-sdk');
StudioSDK.Entities; // or import directly from @clear-street/studio-sdk/resources/entities/entities
```

#### Cleaned up `uploads` exports

As part of the `core` refactor, `@clear-street/studio-sdk/uploads` was moved to `@clear-street/studio-sdk/core/uploads`
...
```typescript
import { type Uploadable, toFile } from '@clear-street/studio-sdk/core/uploads';
```
...
```typescript
// Before
import { APIClient } from '@clear-street/studio-sdk/core';
...
```ts
// Before
StudioSDK.fileFromPath('path/to/file');
...
The `@clear-street/studio-sdk/shims` imports have been removed. Your global types must now be [correctly configured](#minimum-types-requirements).

### `@clear-street/studio-sdk/src` directory removed

Previously IDEs may have auto-completed imports from the `@clear-street/studio-sdk/src` directory, however this
directory was only included for an improved go-to-definition experience and should not have been used at runtime.

If you have any `@clear-street/studio-sdk/src/*` imports, you will need to replace them with `@clear-street/studio-sdk/*`.

```ts
// Before
import StudioSDK from '@clear-street/studio-sdk/src';
