...
import { APIPromise } from '@clear-street/studio-sdk/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['STUDIO_SDK_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: StudioSDK) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['STUDIO_SDK_LOG'] = 'debug';
      const client = new StudioSDK({ logger: logger, bearerToken: 'My Bearer Token' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['STUDIO_SDK_LOG'] = 'not a log level';
      const client = new StudioSDK({ logger: logger, bearerToken: 'My Bearer Token' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'STUDIO_SDK_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
