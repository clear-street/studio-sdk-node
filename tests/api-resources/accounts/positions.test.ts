...
describe('resource positions', () => {
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(studioSDK.accounts.positions.retrieve('x', 'AAPL', { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('list: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(studioSDK.accounts.positions.list('x', { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(StudioSDK.NotFoundError);
  });
  ...
  test('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(studioSDK.accounts.positions.list('x', { page_size: 1, page_token: 'page_token' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(StudioSDK.NotFoundError);
  });
});
